...
// Route debug output to emulator log via AGBPrint, not on-screen.
static void mm_dbg_console_init(void) {
    irqInit();
    irqEnable(IRQ_VBLANK);
    AGBPrintInit();
}
#define MMDBG(...) AGBPrintf(__VA_ARGS__)
#else
#define MMDBG(...)
#endif

#define ARM_CODE   ))
#define IWRAM_CODE , long_call))
...
#ifdef MM_DEBUG
    mm_dbg_console_init();
    MMDBG("[MM] mixlen=%d ratescale=%d tmr0=%d\n", mm_mixlen, mm_ratescale, mm_timerfreq);
    MMDBG("[MM] wavebuffer=%p mixbuffer=%p\n", mm_wavebuffer, mm_mixbuffer);
    MMDBG("[MM] dma1sad=%p dma2sad=%p\n", (void*)REG_DMA1SAD, (void*)REG_DMA2SAD);
#endif
}
