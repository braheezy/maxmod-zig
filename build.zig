...
pub fn build(b: *std.Build) void {
    ...
    // Host tool: mmutil-zig (Zig CLI wrapper calling translate-c modules)
    ...
    //     .optimize = optimize,
    // });
    // // Expose translate-c raw modules used by the CLI
    // const tc_files_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/files_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_samplefix_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/samplefix_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_wav_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/wav_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_adpcm_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/adpcm_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_mod_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/mod_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_s3m_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/s3m_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_it_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/it_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_msl_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/msl_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_mas_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/mas_c_raw_auto.zig"), .target = host_target, .optimize = optimize });
    // const tc_xm_c_raw = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/xm_c_raw.zig"), .target = host_target, .optimize = optimize });

    // mmutil_mod.addImport("tc_files_c_raw", tc_files_c_raw);
    // mmutil_mod.addImport("tc_samplefix_c_raw", tc_samplefix_c_raw);
    // mmutil_mod.addImport("tc_wav_c_raw", tc_wav_c_raw);
    // mmutil_mod.addImport("tc_adpcm_c_raw", tc_adpcm_c_raw);
    // mmutil_mod.addImport("tc_mod_c_raw", tc_mod_c_raw);
    // mmutil_mod.addImport("tc_s3m_c_raw", tc_s3m_c_raw);
    // mmutil_mod.addImport("tc_it_c_raw", tc_it_c_raw);
    // mmutil_mod.addImport("tc_msl_c_raw", tc_msl_c_raw);
    // mmutil_mod.addImport("tc_mas_c_raw", tc_mas_c_raw);
    // mmutil_mod.addImport("tc_xm_c_raw", tc_xm_c_raw);

    // // Shim requires access to some of these imports as well
    // const tc_shim = b.createModule(.{ .root_source_file = b.path("src/mmutil/tc/shim.zig"), .target = host_target, .optimize = optimize });
    // tc_shim.addImport("tc_mas_c_raw", tc_mas_c_raw);
    // tc_shim.addImport("tc_samplefix_c_raw", tc_samplefix_c_raw);
    // tc_shim.addImport("tc_adpcm_c_raw", tc_adpcm_c_raw);
    // mmutil_mod.addImport("tc_shim", tc_shim);
    // const mmutil_exe = b.addExecutable(.{
    //     .name = "mmutil-zig",
    //     .root_module = mmutil_mod,
    // });

    // Do not install mmutil by default; build only when used by an explicit step
    ...
    // Copy the C build's soundbank into the xm_port directory so @embedFile sees the exact same bank
