...
pub export fn mmInitDefault(arg_soundbank: mm_addr, arg_number_of_channels: mm_word) bool {
    ...
    _ = &number_of_channels;
    if (number_of_channels > @as(mm_word, @bitCast(@as(c_int, 32)))) return @as(c_int, 0) != 0;
    var size_of_channel: usize = (@sizeOf(mm_module_channel) +% @sizeOf(mm_active_channel)) +% @sizeOf(mm_mixer_channel);
    _ = &size_of_channel;
    var size_of_buffer: usize = @as(c_uint, @bitCast(MM_MIXLEN_16KHZ)) +% (number_of_channels *% size_of_channel);
    _ = &size_of_buffer;
    mm_init_default_buffer = calloc(@as(usize, @bitCast(@as(c_int, 1))), size_of_buffer);
    if (mm_init_default_buffer == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))) return @as(c_int, 0) != 0;
    var wave_memory: mm_addr = undefined;
    _ = &wave_memory;
    var module_channels: mm_addr = undefined;
    _ = &module_channels;
    var active_channels: mm_addr = undefined;
    _ = &active_channels;
    var mixing_channels: mm_addr = undefined;
    _ = &mixing_channels;
    wave_memory = mm_init_default_buffer;
    module_channels = @as(mm_addr, @ptrFromInt(@as(mm_word, @intCast(@intFromPtr(wave_memory))) +% @as(mm_word, @bitCast(MM_MIXLEN_16KHZ))));
    active_channels = @as(mm_addr, @ptrFromInt(@as(mm_word, @intCast(@intFromPtr(module_channels))) +% (number_of_channels *% @sizeOf(mm_module_channel))));
    mixing_channels = @as(mm_addr, @ptrFromInt(@as(mm_word, @intCast(@intFromPtr(active_channels))) +% (number_of_channels *% @sizeOf(mm_active_channel))));
    var setup: mm_gba_system = mm_gba_system{
        .mixing_mode = @as(c_uint, @bitCast(MM_MIX_16KHZ)),
...
// /opt/devkitpro/devkitARM/arm-none-eabi/include/alloca.h:16:9
...
pub const mixlen = MM_MIXLEN_16KHZ;
