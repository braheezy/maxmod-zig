...
inline fn sample_length_from_data(data: [*c]const u8) u32 {
    // Sample header stores u16 values, not u32 - read as u16 and extend to u32
    const hdr_u16: [*]const u16 = @ptrCast(@alignCast(data));
    const len_ptr = @as([*]const u16, @ptrFromInt(@intFromPtr(hdr_u16) - 12));
    return len_ptr[0];
}
inline fn loop_length_from_data(data: [*c]const u8) i32 {
    // Sample header stores u16 values, not u32 - read as u16 and extend to i32
    const hdr_u16: [*]const u16 = @ptrCast(@alignCast(data));
    const loop_ptr = @as([*]const u16, @ptrFromInt(@intFromPtr(hdr_u16) - 10));
    return @as(i32, @bitCast(@as(u32, loop_ptr[0])));
}
...
pub export fn mmMixerMix(sample_count: u32) linksection(".iwram") callconv(.C) void {
    ...
    if (build_options.xm_debug) {
        debug.printInt("Z_mix_entry", sample_count);
        const gba = @import("gba");
        _ = gba.debug.print("[MIXER] sizeof(mm_mixer_channel)={d} addr=0x{x}\n", .{
            @sizeOf(mm_mixer_channel), @intFromPtr(mm_mix_channels)
        }) catch {};
    }
    ...
    var total_right_volume: u32 = 0;
    {
        ...
        while (channel < mm_mixch_end) : (channel += 1) {
            ...
            // Debug channel 5 sample info
            if (build_options.xm_debug) {
                const ch_idx: usize = @intFromPtr(channel) - @intFromPtr(mm_mix_channels);
                const ch_num = ch_idx / @sizeOf(mm_mixer_channel);
                if (ch_num == 5) {
                    const gba = @import("gba");
                    const hdr_ptr: [*]const u32 = @ptrCast(@alignCast(sample_data));
                    const len_ptr = @as([*]const u32, @ptrFromInt(@intFromPtr(hdr_ptr) - 12));
                    const loop_ptr = @as([*]const u32, @ptrFromInt(@intFromPtr(hdr_ptr) - 8));
                    const len_raw = len_ptr[0];
                    const loop_raw = loop_ptr[0];
                    _ = gba.debug.print("[ZMIX_CH5] src=0x{x} data=0x{x} len_raw=0x{x} loop_raw=0x{x}\n", .{
                        src, @intFromPtr(sample_data), len_raw, loop_raw
                    }) catch {};
                }
            }
            ...
            while (frame_index < frame_count) {
                if (read_position >= sample_length_fp) {
                    ...
                    while (true) {
                        ...
                        if (!(read_position >= sample_length_fp)) break;
                    }
                }
            ...
            if (build_options.xm_debug) {
                ...
                if (ch_idx == 5) {
                    ...
                    _ = gba.debug.print("[ZIG_MIX_CH5] read=0x{x} src=0x{x} vol={d} pan={d} freq={d}\n", .{
                        channel.*.read, channel.*.src, channel.*.vol, channel.*.pan, channel.*.freq
                    }) catch {};
                }
            }
