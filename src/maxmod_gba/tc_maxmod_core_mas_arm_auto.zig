...
// maxmod/source/core/mas_arm.c:359:9: warning: TODO implement translation of stmt class GotoStmtClass

// maxmod/source/core/mas_arm.c:352:1: warning: unable to translate function, demoted to extern
pub extern fn mmUpdateChannel_T0(arg_module_channel: [*c]mm_module_channel, arg_mpp_layer: [*c]mpl_layer_information, arg_channel_counter: mm_byte) linksection(".iwram") void;
...
pub export fn mmGetPeriod(arg_mpp_layer: [*c]mpl_layer_information, arg_tuning: mm_word, arg_note: mm_byte) linksection(".iwram") mm_word {
    ...
    if ((@as(c_int, @bitCast(@as(c_uint, mpp_layer.*.flags))) & (@as(c_int, 1) << @intCast(2))) != 0) {
        // Linear frequency mode uses IT_PitchTable (mm_hword per note); widen to mm_word
        return @as(mm_word, @intCast(IT_PitchTable[note]));
    }
...
pub export fn mmReadPattern(arg_mpp_layer: [*c]mpl_layer_information) linksection(".iwram") mm_bool {
    ...
    while (true) {
        ...
        _ = &read_byte;
        if ((@as(c_int, @bitCast(@as(c_uint, read_byte))) & @as(c_int, 127)) == @as(c_int, 0)) break;
        var pattern_flags: mm_word = 0;
        _ = &pattern_flags;
        var chan_num: mm_byte = @as(mm_byte, @bitCast(@as(i8, @truncate((@as(c_int, @bitCast(@as(c_uint, read_byte))) & @as(c_int, 127)) - @as(c_int, 1)))));
        _ = &chan_num;
        if (@as(c_int, @bitCast(@as(c_uint, chan_num))) >= @as(c_int, @bitCast(@as(c_uint, mpp_nchannels)))) return 0;
...
pub fn mmChannelStartACHN(arg_module_channel: [*c]mm_module_channel, arg_active_channel: [*c]mm_active_channel, arg_mpp_layer: [*c]mpl_layer_information, arg_channel_counter: mm_byte) linksection(".iwram") callconv(.c) mm_byte {
    ...
    var instrument: ?*mm_mas_instrument = mpp_InstrumentPointer(mpp_layer, @as(mm_word, @bitCast(@as(c_uint, module_channel.*.inst))));
    _ = &instrument;
    if (instrument.*.is_note_map_invalid != 0) {
        if (active_channel != null) {
            active_channel.*.sample = @as(mm_byte, @bitCast(@as(i8, @truncate(@as(c_int, @bitCast(@as(c_uint, instrument.*.note_map_offset))) & @as(c_int, 255)))));
        }
        module_channel.*.note = module_channel.*.pnoter;
    } else {
        var note_map: [*c]mm_hword = @as([*c]mm_hword, @ptrFromInt(@as(mm_word, @intCast(@intFromPtr(instrument))) +% @as(mm_word, @bitCast(@as(c_uint, instrument.*.note_map_offset)))));
        _ = &note_map;
        var notemap_entry: mm_hword = note_map[module_channel.*.pnoter];
        _ = &notemap_entry;
        module_channel.*.note = @as(mm_byte, @bitCast(@as(i8, @truncate(@as(c_int, @bitCast(@as(c_uint, notemap_entry))) & @as(c_int, 255)))));
        if (active_channel != null) {
            active_channel.*.sample = @as(mm_byte, @bitCast(@as(i8, @truncate(@as(c_int, @bitCast(@as(c_uint, notemap_entry))) >> @intCast(8)))));
        }
    }
