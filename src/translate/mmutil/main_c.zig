...
pub extern fn MSL_Create(argv: [*c][*c]u8, argc: c_int, output: [*c]u8, header: [*c]u8, verbose: @"bool") c_int;
pub extern fn Write_NDS() void;
pub extern fn Validate_NDS(filename: [*c]u8, output_size: c_int) void;
...
pub extern fn kiwi_start() void;
pub export var ignore_sflags: @"bool" = @import("std").mem.zeroes(@"bool");
pub export var number_of_inputs: c_int = @import("std").mem.zeroes(c_int);
pub export fn print_usage() void {
    _ = printf("mmutil");
}
pub export fn print_error(arg_err: c_int) void {
    var err = arg_err;
    _ = &err;
    while (true) {
        switch (err) {
            @as(c_int, 1) => {
                _ = printf("Invalid module!\n");
                break;
            },
            @as(c_int, 2) => {
                _ = printf("Too many arguments!\n");
                break;
            },
            @as(c_int, 3) => {
                _ = printf("No input file!\n");
                break;
            },
            @as(c_int, 4) => {
                _ = printf("Unable to write file!\n");
                break;
            },
            @as(c_int, 11) => {
                _ = printf("Cannot parse input filename!\n");
                break;
            },
            else => {},
        }
...
pub export fn GetYesNo() c_int {
    var c: u8 = 0;
    _ = &c;
    c = @as(u8, @bitCast(@as(i8, @truncate(tolower(getchar())))));
    while (getchar() != @as(c_int, '\n')) {}
    while ((@as(c_int, @bitCast(@as(c_uint, c))) != @as(c_int, 'y')) and (@as(c_int, @bitCast(@as(c_uint, c))) != @as(c_int, 'n'))) {
        _ = printf("Was that a yes? ");
        c = @as(u8, @bitCast(@as(i8, @truncate(tolower(getchar())))));
        while (getchar() != @as(c_int, '\n')) {}
    }
    return if (@as(c_int, @bitCast(@as(c_uint, c))) == @as(c_int, 'y')) @as(c_int, 1) else @as(c_int, 0);
}
...
// src/translate/mmutil/main_prep.c:95:5: warning: unable to translate function, demoted to extern
