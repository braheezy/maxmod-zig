...
pub export fn mmInitDefault(arg_soundbank: mm_addr, arg_number_of_channels: mm_word) bool {
    ...
    _ = &size_of_channel;
    var size_of_buffer: usize = @as(usize, @bitCast(@as(c_long, MM_MIXLEN_16KHZ))) +% (@as(usize, @bitCast(@as(c_ulong, number_of_channels))) *% size_of_channel);
    _ = &size_of_buffer;
    mm_init_default_buffer = calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), size_of_buffer);
    ...
    active_channels = @as(mm_addr, @ptrFromInt(@as(c_ulong, @bitCast(@as(c_ulong, @as(mm_word, @intCast(@intFromPtr(module_channels)))))) +% (@as(c_ulong, @bitCast(@as(c_ulong, number_of_channels))) *% @sizeOf(mm_module_channel))));
    mixing_channels = @as(mm_addr, @ptrFromInt(@as(c_ulong, @bitCast(@as(c_ulong, @as(mm_word, @intCast(@intFromPtr(active_channels)))))) +% (@as(c_ulong, @bitCast(@as(c_ulong, number_of_channels))) *% @sizeOf(mm_active_channel))));
...
pub export fn mmInit(arg_setup: [*c]mm_gba_system) bool {
    var setup = arg_setup;
    _ = &setup;
    mp_solution = @as([*c]msl_head, @ptrCast(@alignCast(setup.*.soundbank)));
    mmSampleCount = @as(mm_word, @bitCast(@as(c_uint, mp_solution.*.head_data.sampleCount)));
    mmModuleCount = @as(mm_word, @bitCast(@as(c_uint, mp_solution.*.head_data.moduleCount)));
    ...
    mmMixerInit(setup);
    mm_ch_mask = (@as(c_uint, 1) << @intCast(mm_num_ach)) -% @as(c_uint, @bitCast(@as(c_int, 1)));
...
pub export fn mmFrame() void {
    ...
    while (true) {
        ...
        _ = &sample_num;
        // translate-c emitted an anonymous union; refer to the named field explicitly
